...
class SupportRequestInput(BaseModel):
    ...
    subject: StrictStr = ...
    __properties = ["device_id", "href", "message", "priority", "project_id", "subject"]

    @validator('priority')
    def priority_validate_enum(cls, v):
        if v is None:
            return v
        if v not in ('urgent', 'high', 'medium', 'low'):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SupportRequestInput:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SupportRequestInput:
        """Create an instance of SupportRequestInput from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return SupportRequestInput.parse_obj(obj)

        _obj = SupportRequestInput.parse_obj({
