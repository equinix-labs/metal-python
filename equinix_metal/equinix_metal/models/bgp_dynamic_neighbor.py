...
class BgpDynamicNeighbor(BaseModel):
    ...
    __properties = ["bgp_neighbor_asn", "bgp_neighbor_range", "created_at", "created_by", "href", "id", "metal_gateway", "state", "updated_at"]

    @validator('state')
    def state_validate_enum(cls, v):
        if v is None:
            return v
        if v not in ('active', 'deleting', 'pending', 'ready'):
